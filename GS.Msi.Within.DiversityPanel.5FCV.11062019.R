...
#data_hmp <- read.delim("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/Msi.Geno.5pmaf.90missing.54667snps.538lines.hmp.txt", head=F)
#myG <- data_hmp
...
phen.taxa <- read.csv("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/Msi.Raw.Selection.Index.Phenotypes.538lines.11052019.csv", header=T)
phen.taxa$Taxa <- as.character(phen.taxa$Taxa)
head(phen.taxa)
phen.taxa <- phen.taxa[,-c(6,7,11)] # Remove pop column
head(phen.taxa)
...
Msi.polyRAD.geno.2[1:6,1:6]
#Msi.polyRAD.geno.3 <- Msi.polyRAD.geno.2-1
#A.Aher <- A.mat(Msi.polyRAD.geno.3)
...
#  model <- mixed.solve(y,K=A.Aher) #$A
...
#write.table(h2, "/Users/omo/Box Sync/Desktop/PostDoc/Manuscript2/Data/Msi.trait.heritablility.polRAD.11112019.csv", sep=",", quote=F, row.names = F, col.names = T)

her.vec <- read.csv('http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/DiversityPanel.GS/Msi/Msi.trait.heritablility.polRAD.11112019.csv', header=T)
...
PCA.Msi <- read.delim("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/Msi.polyRAD.estimated.PCA.txt", header=T)
dim(PCA.Msi)
PCA.Msi[1:6,1:6]
names(PCA.Msi)[1] <- "Taxa" # Select 4 PCs as optimal
...
Within.Diversity.Panel.GS.Pipeline(phenames=phenames, polyRAD.Geno=polyRAD.Geno, m_train.pheno=com.taxa, her.vec=her.vec, dat=Msi.polyRAD.geno.2, PCA.info=PCA.Msi, Opt.PCA=Opt.PCA, 
...
phenames=phenames; polyRAD.Geno=polyRAD.Geno; m_train.pheno=com.taxa; her.vec=her.vec; dat=Msi.polyRAD.geno.2; num.vec=num.vec; s=s; Iter=Iter; PCA.info=PCA.Msi; Opt.PCA=Opt.PCA;
burn=burn; species=species; k=k; itrn=itrn; cycles = cycles;trn.app=trn.app; number.of.folds=number.of.folds; geno.diplo.tetra=geno.diplo.tetra; 
Groups.3.Pop3=Groups.3.Pop3; date=date
