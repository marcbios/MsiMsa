...
data_hmp <- read.delim("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/Msi.Geno.5pmaf.90pmissing.hmp.txt", head=F)
myG <- data_hmp
...
Msi.Pheno <- read.csv("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/com.taxa.numeric.pheno.geno.Msi.Msa.csv", header=T)
Msi.Pheno$Taxa <- as.character(Msi.Pheno$Taxa)
library(rrBLUP)
phenames <- colnames(Msi.Pheno[,-1])
...
com.tax <- merge(Msi.Pheno, geno.names, by="Taxa")
...
write.table(train.pop, "~/public_html/effect_simulations/PostDoc/Manuscript2/Data/DiversityPanel.GS/Msi/Msi.Div.CDmean.CalibrationSet.txt", sep="\t", quote=F, row.names = F, col.names = F)
...
write.table(test.pop, "~/public_html/effect_simulations/PostDoc/Manuscript2/Data/DiversityPanel.GS/Msi/Msi.Div.CDmean.TestSet.txt", sep="\t", quote=F, row.names = F, col.names = F)
Geno <- m_train1+1
...
write.table(h2, "Msi.trait.heritablility.csv", sep=",", quote=F, row.names = F, col.names = T)
...
#setwd("/homes/omo/public_html/effect_simulations/PostDoc/Manuscript2/Data/DiversityPanel.GS/msi")
...
for(p in 1:length(phenames)){
        ...
        print(paste("------ Calibration set size being evaluated : ", num.vec[n], " !!!!!!!!!!!---------------", sep = ""))
            ...
            write.table(SampleOptimiz, paste("Msi.CDmeanMax1.DivPanel.3000", phenames[p],"size-n:",num.vec[n],"txt", sep="."), sep="\t", quote=F, row.names=F, col.names=F)
            SampleOptimiz.names <- ind.names[SampleOptimiz]
            write.table(SampleOptimiz.names, paste("Msi.CDmeanMax1.DivPanel.itrn3000.individuals", phenames[p],"size-n:",num.vec[n],"txt", sep="."), sep="\t", quote=F, row.names=F, col.names=F)
            
            # End
            pdf(paste("Msi.CDmeanMax1.DivPanel.3000", phenames[p],"size-n:",num.vec[n],"pdf", sep="."))
...
rm(list=ls())
setwd("~/Desktop/PostDoc/Manuscript2/Data/")
library(multtest)
library(gplots)
library(LDheatmap)
library(genetics)
...
test.set.CDmean <- read.delim("/Users/omo/Desktop/PostDoc/Manuscript2/Data/Msi.Div.CDmean.TestSet.txt", header=F)
...
trainingset.CDmean <- read.delim("/Users/omo/Desktop/PostDoc/Manuscript2/Data/Msi.Div.CDmean.CalibrationSet.txt", header=F)
...
her.vec <- read.csv('Msi.trait.heritablility.csv', header=T)
...
for(l in 1:length(phenames)){
  ...
  for(n in 1:length(num.vec)){
        ...
        CDmean.names.Msi <- read.delim(paste(specie.name[2],"CDmeanMax1.itrn3000.individuals", phenames[l],"size-n:",num.vec[n],"txt", sep="."), header=F)
        ...
        trt.vec1 <- as.character(CDmean.names.Msi$V1)
      ...
      for(j in 1:cycles){
            ...
            Sv.accuracy=matrix(nrow=cycles,ncol=k+20)
        ...
        write.table(GS.Results.MsiMsa , paste("GS.Results.Msi", phenames[l], num.vec[n], ".txt",sep="_"), sep="\t", quote=F, row.names = F, col.names = F)
      ...
      write.table(GS.Results.MsiMsa.TRSvec , paste("GS.Results.Msi.TRSvec", phenames[l], ".txt",sep="_"), sep="\t", quote=F, row.names = F, col.names = F)
...
save.image("GS.GS.Results.Msi.TRSvec.rda")
write.table(GS.Results.MsiMsa.TRSvec.Traits, "GS.Results.Msi.TRSvec.Traits.03172019.csv", sep=",", quote=F, row.names=F, col.names=T)
