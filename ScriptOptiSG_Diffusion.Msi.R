...
# Data required
##########################
#Msi.Pheno <- read.csv("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/Msi.Pheno.csv", header=T)
Msi.Pheno <- read.csv("http://people.beocat.ksu.edu/~omo/effect_simulations/PostDoc/Manuscript2/Data/Msi.Pheno.csv", header=T)
  ...
  out <- marker_h2(com.tax[,phenames[p]], com.tax$Taxa, covariates = NULL, A, alpha = 0.05, eps = 1e-06, max.iter = 100, fix.h2 = FALSE)
      ...
      for(n in 1:length(num.vec)){
        print(paste("------ Calibration set size being evaluated : ", num.vec[n], " !!!!!!!!!!!---------------", sep = ""))
            ...
            cpt=0
            while (cpt2<itrn) {
            print(paste("--- Trait: ",phenames[p], " --- Calibration set: ", num.vec[n]," --- Iteration: ", cpt2, " out of ", itrn," ---", sep = ""))
            ...
            write.table(SampleOptimiz, paste("Msi.CDmeanMax1.3000", phenames[p],"size-n:",num.vec[n],"txt", sep="."), sep="\t", quote=F, row.names=F, col.names=F)
            SampleOptimiz.names <- ind.names[SampleOptimiz]
            write.table(SampleOptimiz.names, paste("Msi.CDmeanMax1.itrn3000.individuals", phenames[p],"size-n:",num.vec[n],"txt", sep="."), sep="\t", quote=F, row.names=F, col.names=F)
            
            # End
            pdf(paste("Msi.CDmeanMax1.3000", phenames[p],"size-n:",num.vec[n],"pdf", sep="."))
